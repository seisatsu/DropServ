...
class DropDatabase(SQLiteBase):
    ...
    def getUser(self, userName):
        s = self.runCustomQuery("SELECT * FROM users WHERE email=?", (userName,), requiresCommit=False)
        if not s:
            return None
        else:
            return {
    ...
    def getUserByKey(self, ak):
        s = self.runCustomQuery("SELECT * FROM users WHERE apikey=?", (ak,), requiresCommit=False)
        if not s:
            return None
        else:
            return {
    ...
    def insertFileRecord(self, table):
        self.runCustomQuery("INSERT INTO drops VALUES (NULL,?,?,?,?,?,0,?)", (table["email"], table["drop"], table["mimetype"], table["realname"],
                                                                              table["size"], table["ts"]), lock=True)

    def setup_db(self):
        self.runCustomQuery("CREATE TABLE drops (id INTEGER PRIMARY KEY AUTOINCREMENT, owner TEXT, url TEXT, mimetype TEXT, filename TEXT," \
                            "size INT, views INT, timestamp INT)", lock=True)
        self.runCustomQuery("CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT, passwordHash TEXT, apikey TEXT, usage INT)", lock=True)
