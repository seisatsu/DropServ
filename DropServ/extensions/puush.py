    ...
    def __init__(self, rootClass):
        ...
        def history(self, request):
            if request.post and "k" in request.form:
                user = self.instance.Database.getUser("apikey", request.form["k"].value.decode("utf-8"))
                if not user:
                    return Objects.Response(s="200 OK", h={"Content-Type": "text/html"}, r="-1")
                else:
                    lastdrops = self.instance.Database.getDropsByUser(user["email"], limit=10, sort_by="timestamp")
                    lastdrops.reverse()
